...
async def one_wallet_swaps(wallet: dict, index: int):
    num_of_swaps = random.randint(1, 2)
    num_of_swaps = 1
    # num_of_swaps = 2 if int(wallet['all_swaps']) < 4 else 1
    logger.info(f"SWAPS | index = {index} | {wallet['public_key']} | num = {num_of_swaps} |")
    await asyncio.sleep(int(1 + index * random.uniform(5, 5)) * 60)

    for num in range(num_of_swaps):
        await _gas_check()
        start = RandomSwaps(wallet=wallet, index=index, minuts=random.uniform(5, 5) if num_of_swaps > 1 else 0)
