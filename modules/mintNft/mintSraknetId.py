        ...
        try:
            while True:
                try:
                    ...
                    mint_router_address = int(STARKNET["starknetid_router"], 16)
            ...
            starknet_id = random.randint(10**10, 10**13-1)
            mint_call = mint_contract.functions["mint"].prepare(starknet_id=starknet_id)
            calls = mint_call
            ...
            await update_mint_nft(public_key=self.__wallet["public_key"], nft_id=starknet_id)
            await session.close()
...
async def main():
    ...
    start = StarkNetId(wallet=wallet)
    await start.mint()
