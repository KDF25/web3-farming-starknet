...
from constants.data import STARKNET, tenkswap_abi
        ...
        try:
            while True:
                try:
                    ...
                    _10kSwap_router_address = int(STARKNET["10kSwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=_10kSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            _10kSwap_contract = Contract(
                address=_10kSwap_router_address,
                provider=self.__account,
                abi=tenkswap_abi,
            )

            result = await _10kSwap_contract.functions["getAmountsOut"].call(amountIn=amountIn,
            ...
            swap_call = _10kSwap_contract.functions["swapTokensForExactTokens"].prepare(amountOut,
                                                                                        amountIn,
                                                                                        [from_token_address,
                                                                                         to_token_address],
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="10kSwap")
            await session.close()
            ...
            ex = str(ex).replace("\n", " - ")
            await session.close()
            logger.error(f'ERROR_3  | index = {self.__index} | {self.__wallet["public_key"]} | {self.__from_token} --> {self.__to_token} | {ex}')
...
async def main():
    ...
    start = TenKSwap(wallet=wallet, from_token="WBTC", to_token="ETH", amount=10000)
    await start.swap()
