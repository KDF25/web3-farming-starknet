...
class ProtossSwap:
    ...
    async def _get_amountOut(self, amountIn: int):
        ...
        result = await jediSwap_contract.functions["get_amounts_out"].call(amountIn=amountIn,
                                                                           path=[
                                                                               int(STARKNET[self.__from_token], 16),
                                                                               int(STARKNET[self.__to_token], 16)
                                                                           ])

        self.__amountOut = result.amounts[1]
        self.__amountOut = int(self.__amountOut * (1 - slippage / 100))
        ...
        try:
            while True:
                try:
                    ...
                    protossSwap_router_address = int(STARKNET["protossSwap_router"], 16)
                    from_token_address = int(STARKNET[self.__from_token], 16)
                    to_token_address = int(STARKNET[self.__to_token], 16)

                    eth_contract = await get_from_token_contract(account=self.__account, from_token="ETH", address=int(STARKNET["ETH"], 16))
                    ...
                    ex = str(ex).replace("\n", " - ")
                    await session.close()
                    logger.error(f'ERROR_1  | index = {self.__index} | {self.__wallet["public_key"]} | {self.__from_token} --> {self.__to_token} | {self.__from_token} | {ex}')
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=protossSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            protossSwap_contract = await Contract.from_address(
                address=protossSwap_router_address,
                provider=self.__account,
                proxy_config=True
            )

            await self._get_amountOut(amountIn=amountIn)
            deadline = int(time.time()) + 1200

            swap_call = protossSwap_contract.functions["swapExactTokensForTokens"].prepare(amountIn=amountIn,
                                                                                           amountOutMin=self.__amountOut,
                                                                                           path=[from_token_address,
                                                                                                 to_token_address],
                                                                                           to=self.__account.address,
                                                                                           deadline=deadline)
            ...
            while True:
                try:
                    ...
                    ex = str(ex).replace("\n", " - ")
                    logger.error(f'ERROR_2  | index = {self.__index} | {self.__wallet["public_key"]} | {self.__from_token} --> {self.__to_token} | {self.__from_token} | {ex}')
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="protossSwap")
            await session.close()
            ...
            ex = str(ex).replace("\n", " - ")
            await session.close()
            logger.error(f'ERROR_3  | index = {self.__index} | {self.__wallet["public_key"]} | {self.__from_token} --> {self.__to_token} | {self.__from_token} | {ex}')

            if self.__repeat < max_tries and "list index out of range" not in str(ex):
...
async def main():
    ...
    start = ProtossSwap(wallet=wallet, from_token="USDC", to_token="ETH", amount=10**200)
    await start.swap()
