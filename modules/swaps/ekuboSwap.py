...
from constants.data import STARKNET, ANVU_EXCHANGE_ADDRESSES
from database.main import update_swap
from modules.utils import approve_token, get_account, get_from_token_contract, deposit_balance

logger.remove()
custom_name = "EkuvoSwap"
        ...
        try:
            while True:
                try:
                    ...
                    anvuSwap_router_address = int(STARKNET["anvuSwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=anvuSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            anvuSwap_contract = await Contract.from_address(
                address=anvuSwap_router_address,
                provider=self.__account,
                proxy_config=True
            )
            ...
            exchange_address = random.choice(ANVU_EXCHANGE_ADDRESSES)
            exchange_address = int(STARKNET[exchange_address], 16)
            swap_call = anvuSwap_contract.functions["multi_route_swap"].prepare(token_from_address=from_token_address,
                                                                                token_from_amount=amountIn,
                                                                                token_to_address=to_token_address,
                                                                                token_to_amount=self.__amountOut,
                                                                                token_to_min_amount=self.__amountOutMin,
                                                                                beneficiary=self.__account.address,
                                                                                integrator_fee_amount_bps="0",
                                                                                integrator_fee_recipient="0",
                                                                                routes=[{
                                                                                    "token_from": from_token_address,
                                                                                    "token_to": to_token_address,
                                                                                    "exchange_address": exchange_address,
                                                                                    "percent": 100,
                                                                                    "additional_swap_params": [from_token_address,
                                                                                                               to_token_address]
                                                                                }])
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="AnvuSwap")
            await session.close()
...
async def main():
    ...
    start = EkuboSwap(wallet=wallet, from_token="USDC", to_token="ETH", amount=1)
    await start.swap()
