...
from constants.data import STARKNET, fibrousswap_abi, FIBROUSSWAP
from database.main import update_swap, current_pool_fibrousSwap
from modules.utils import approve_token, get_account, get_from_token_contract, deposit_balance

logger.remove()
custom_name = "FibrousSwap"
...
class FibrousSwap:
    ...
    async def _get_amountOut(self, amountIn: int):
        ...
        self.__amountOutMin = int(self.__amountOut * (1 - slippage / 100))

    async def _get_swap(self):
        while True:
            swap = random.choice(list(FIBROUSSWAP.keys()))
            self.__protocol = FIBROUSSWAP[swap]
            row = await current_pool_fibrousSwap(from_token=self.__from_token, to_token=self.__to_token, swap=swap)
            if row[swap] != "error":
                self.__pool_address = int(row[swap], 16)
                return
        ...
        try:
            while True:
                try:
                    ...
                    fibrousSwap_router_address = int(STARKNET["fibrousSwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=fibrousSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            fibrousSwap_contract = Contract(
                address=fibrousSwap_router_address,
                provider=self.__account,
                abi=fibrousswap_abi,
            )

            await self._get_amountOut(amountIn=amountIn)
            await self._get_swap()

            swap_call = fibrousSwap_contract.functions["swap"].prepare(swaps=[{"token_in": from_token_address,
                                                                               "token_out": to_token_address,
                                                                               "rate": 1000000,
                                                                               "protocol": self.__protocol,
                                                                               "pool_address": self.__pool_address}],
                                                                       params={"token_in": from_token_address,
                                                                               "token_out": to_token_address,
                                                                               "amount": amountIn,
                                                                               "min_received": self.__amountOutMin,
                                                                               "destination": self.__account.address})
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="fibrousswap")
            await session.close()
...
async def main():
    ...
    start = FibrousSwap(wallet=wallet, from_token="WBTC", to_token="ETH", amount=1)
    await start.swap()
