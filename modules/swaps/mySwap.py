...
from database.main import update_swap, current_pool_mySwap, current_wallet
        ...
        try:
            while True:
                try:
                    ...
                    mySwap_router_address = int(STARKNET["mySwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=mySwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            mySwap_contract = await Contract.from_address(
                address=mySwap_router_address,
                provider=self.__account,
                proxy_config=True
            )

            pool = await current_pool_mySwap(from_token=self.__from_token, to_token=self.__to_token)
            await self._get_amountOut(amountIn=amountIn)

            swap_call = mySwap_contract.functions["swap"].prepare(
                pool_id=pool["pool_id"],
                token_from_addr=from_token_address,
                amount_from=amountIn,
                amount_to_min=self.__amountOut
            )
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="MySwap")
            await session.close()
...
async def main():
    ...
    start = MySwap(wallet=wallet, from_token="USDT", to_token="ETH", amount=10**200)
    await start.swap()
