...
from constants.data import STARKNET, starkexswap_abi
        ...
        try:
            while True:
                try:
                    ...
                    starkExSwap_router_address = int(STARKNET["starkExSwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=starkExSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            starkExSwap_contract = Contract(
                address=starkExSwap_router_address,
                provider=self.__account,
                abi=starkexswap_abi,
            )

            result = await starkExSwap_contract.functions["getAmountsOut"].call(amountIn=amountIn,
            ...
            swap_call = starkExSwap_contract.functions["swapTokensForExactTokens"].prepare(amountOut=amountOut,
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="starkexswap")
            await session.close()
...
async def main():
    ...
    start = StarkExSwap(wallet=wallet, from_token="WBTC", to_token="ETH", amount=1)
    await start.swap()
