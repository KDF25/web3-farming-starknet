...
from constants.data import STARKNET, sithswap_abi
        ...
        try:
            while True:
                try:
                    ...
                    sithSwap_router_address = int(STARKNET["sithSwap_router"], 16)
            ...
            calls = await approve_token(from_token_contract=from_token_contract, amount=amountIn,
                                        spender=sithSwap_router_address, owner=self.__account.address,
                                        from_token=self.__from_token, to_token=self.__to_token, index=self.__index)

            sithSwap_contract = Contract(
                address=sithSwap_router_address,
                provider=self.__account,
                abi=sithswap_abi,
            )

            result = await sithSwap_contract.functions["getAmountOut"].call(amount_in=amountIn,
                                                                            token_in=from_token_address,
                                                                            token_out=to_token_address)

            amountOut = result[0]
            amountOut = int(amountOut * (1 - slippage / 100))

            deadline = int(time.time()) + 1200

            swap_call = sithSwap_contract.functions["swapExactTokensForTokensSupportingFeeOnTransferTokens"]. \
                prepare(amount_in=amountIn,
                        amount_out_min=amountOut,
                        routes=[{"from_address": from_token_address, "to_address": to_token_address, "stable": 0}],
            ...
            await update_swap(public_key=self.__wallet['public_key'], amount=amountIn / 10 ** token_decimals,
                              balance=balance, from_token=self.__from_token, to_token=self.__to_token, Swap="sithswap")
            await session.close()
...
async def main():
    ...
    start = SithSwap(wallet=wallet, from_token="USDC", to_token="ETH", amount=1000)
    await start.swap()
