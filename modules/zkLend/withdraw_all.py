...
class ZkLendWithdraw:
    def __init__(self, wallet: dict, token: str = None, index: int = 0, minuts: [float, int] = 0) -> None:
        self.__repeat = 0
        self.__account = None
        self.__wallet = wallet
        self.__token = self.__wallet['zklend'].get("deposit_token") if token is None else token
        self.__index = index
        ...
        try:
            while True:
                try:
                    ...
                    token_address = int(STARKNET[self.__token], 16)
            ...
            withdraw_call = zklend_contract.functions["withdraw_all"].prepare(
                token=token_address,
            )
            ...
            route.append(f"{custom_name}|{self.__token}")
            self.__wallet['zklend']["withdraw_count"] = self.__wallet['zklend'].get("withdraw_count", 0) + 1
            self.__wallet['zklend']["total_volume"] = self.__wallet['zklend'].get("total_volume", 0) + self.__wallet['zklend'].get("deposit", 0)
...
async def main():
    ...
    start = ZkLendWithdraw(wallet=wallet, token="ETH")
    await start.start()
