...
class Lending:
    ...
    async def start(self):
        ...
        deposit = ZkLendDeposit(wallet=self.__wallet, from_token="ETH", index=self.__index, minuts=random.randint(10, 20))
        await deposit.start()
        ...
        if self.__type == "REPAY":
            borrow = ZkLendBorrow(wallet=wallet, from_token="ETH", to_token=self.__borrow_token, index=self.__index,
                                  minuts=random.randint(10, 20))
            await borrow.start()
            wallet = await current_wallet(public_key=wallet['public_key'])

            repay = ZkLendRepayAll(wallet=wallet, token=self.__borrow_token, index=self.__index, minuts=random.randint(10, 20))
            await repay.start()
            wallet = await current_wallet(public_key=wallet['public_key'])
    ...
    async def _check_borrow_token(self):
        ...
        if token_balance <= 0.2 * 10 ** token_decimals:
            swap = RandomSwaps(wallet=self.__wallet, from_token="ETH", to_token=self.__borrow_token,
                               index=self.__index, minuts=random.randint(5, 10))
            await swap.start()
...
async def one_wallet_lending(wallet: dict, index: int):
    ...
    while wallet['zklend']["total_volume"] <= ETH_VOLUME:
        lending = Lending(wallet=wallet, _type=_type, borrow_token=borrow_token, index=index, minuts=random.randint(0, 0))
        await lending.start()
        wallet = await current_wallet(public_key=wallet['public_key'])
        _type = random.choice(TYPE)
        borrow_token = random.choice(BORROW_TOKEN)
